...
- 2026-04-10: Operationally recovered a stuck worker install on production for VPS `89.117.22.183`. Inspection showed the repository and runtime had already been cloned, but `youtube-upload-worker.service` was left `inactive (dead)` with no journal output, so the worker never reached the final online state. Starting the service manually on the VPS allowed `worker-03` to register and return to normal polling/claim flow.
- 2026-04-09: Documented the external `D:\LiveStreamBOT-master` livestream architecture for the new `live-youtube` branch. The old system is centered on an ASP.NET API plus SignalR bot model where a standalone bot receives `RenderCommand` jobs, pulls media from cloud links, streams to YouTube RTMP with `ffmpeg`, and coordinates backup-stream failover plus Telegram alerts. No code was merged yet; this pass only established the integration baseline against the current Python worker/control-plane design.
- 2026-04-09: Added the first integrated live-stream admin workspace on branch `live-youtube` without introducing a second UI system. The admin shell now exposes top-level `Upload` / `Live Stream` workspace tabs plus a new sidebar entry `Điều phối live stream`; the user and BOT management pages can switch into `workspace=live`, preserving old live-app table schema for user/BOT management while still using the current control-plane visual language and manager-scope logic. A new `admin/live_workspace.html` page also lays out the old live stream list columns as the control-plane landing page for live management, ready for the future runtime/live-job integration.
- 2026-04-09: Refined the live admin workspace after real browser audit. `backend/app/store.py` and `backend/app/routers/web.py` now keep `workspace=live` consistent across sidebar links, brand link, `Danh sách Kênh`, `Danh sách Render`, and workspace tabs; live user management no longer exposes delete on the last admin row; and the live user/BOT tables were re-laid out to match the current app visual language with compact info cells and inline row actions.
- 2026-04-09: Simplified live capacity modeling in admin management. The control-plane no longer distinguishes `1080/4K` in live workspace tables: `backend/app/store.py` now collapses those legacy labels into one `Luồng live stream` metric plus `Backup`, while `backend/app/templates/admin/user_index.html` and `backend/app/templates/admin/worker_index.html` reflect the merged live-stream column and updated BOT type chips. Also fixed legacy avatar fallbacks to use `/static/admin-themes/...`, removing the remaining browser 404 on the live render workspace.
- 2026-04-09: Compressed the live user/BOT tables further to remove horizontal scrolling. The live user table now forces the `Luồng live stream` and `Backup` headers into stacked two-line labels and uses fixed layout with wrapped action controls, while the live BOT table drops the `Manager` column, removes the worker name from the `BOT` column, shortens `space_text` to the `130.2/512.0GB` form, and updates the live polling row renderer to match the narrower schema.
- 2026-04-09: Removed the duplicated secondary user tab bar from the integrated live/upload admin shell and promoted only `Tạo user`, `Manager`, and `Admin` into the primary workspace strip alongside `Upload` and `Live Stream`. The live user table now hides the extra `Live scope` meta line, keeps action buttons on a single row, and narrows the `Manager`/`BOT` columns; the live BOT table now shows the bot IP plus owning manager in the `BOT` column, hides the internal `worker-01/02` label and legacy type badge, and keeps the compact single-line action layout.
- 2026-04-09: Tuned column widths again for readability and action balance. The live BOT table now constrains `CPU` aggressively, slightly trims the `Tác vụ` column, and reduces action-button padding so `DS User / Sửa / Xóa` no longer defines an oversized action lane. The shared user table template now also applies explicit width classes for `Vai trò`, `Manager`, `Telegram`, `BOT`, and the upload-only `Kênh`/action columns so both `workspace=live` and `workspace=upload` keep more consistent proportions.
- 2026-04-09: Reverted the `workspace=upload` user list table back to the `main` branch proportions inside `backend/app/templates/admin/user_index.html`. This removes the temporary upload-specific width overrides so the `Cập nhật` date column and the rest of the upload user table follow the original main-branch rhythm again, while leaving the live-workspace table untouched.
- 2026-04-09: Added explicit pixel-width classes for the remaining hard-to-tune live user columns in `backend/app/templates/admin/user_index.html`: `stt-col`, `live-col`, and `backup-col`. The live table header now binds those classes directly so future spacing tweaks can be done by editing one CSS block instead of mixed Tailwind width utilities and implicit auto-sizing.
- 2026-04-09: Completed the live user-table width map in `backend/app/templates/admin/user_index.html` by adding explicit `info-col`, `updated-col`, and `action-col` classes and binding them to `Thông tin user`, `Cập nhật`, and `Tác vụ`. The main live user columns can now all be tuned from one CSS block with pixel values.
- 2026-04-09: Rebuilt `backend/app/templates/admin/live_workspace.html` from a placeholder landing screen into a real live-stream control surface based on the legacy app’s configuration flow. The new page keeps the current admin visual system, removes the old `1080/4K` split, adds a structured form for stream name, scoped user, primary/backup BOT, stream key, media URLs, and schedule, and redesigns the `Danh sách stream đang live` table to match the render-list language with merged columns and cover-thumbnail support. `backend/app/store.py` now supplies the live form with scoped user/BOT options and helper cards instead of only shortcut links.
- 2026-04-09: Thêm route user `/app/live`, thêm mục sidebar `Điều phối Live`, dựng màn user live workspace mới theo style `final_user_ui - Copy - Copy.html`, và loại bỏ các panel admin dư thừa khỏi flow live phía user.
- 2026-04-09: Tinh gọn tiếp màn user `/app/live` theo app live cũ: bỏ `Chọn kênh` và panel `My Channel`, chia `Live Config` thành 2 panel ngang `Nguồn phát` và `BOT + lịch live`, đồng thời xóa JS lọc channel không còn dùng để flow tạo luồng chỉ còn media, BOT, stream key và lịch live.
- 2026-04-09: Tiếp tục polish user `/app/live`: lưu backup template kiểu card cũ tại `backend/app/templates/user_live_dashboard.card-backup.html`, chuyển `Live Config` sang panel phẳng có vách chia dọc, và thay 2 dropdown BOT bằng custom select có search/check-state để bám visual của app user hiện tại.
- 2026-04-09: Tinh tiếp panel phẳng của user `/app/live`: bỏ đường kẻ ngang đầu vùng `Nguồn phát / BOT và lên lịch` và kéo nội dung 2 cột lên sát hơn để form gọn, ít khoảng thừa hơn.
- 2026-04-09: Cập nhật cụm lịch trên user `/app/live`: thêm note đỏ `* Bắt buộc` cho thời gian bắt đầu, thêm lưu ý `Live 24/7 thì không chọn thời gian kết thúc` cho ô kết thúc, và để cả hai datetime field ở trạng thái placeholder trống mặc định.
- 2026-04-09: Dọn tiếp copy user `/app/live`: đổi note `Kết thúc live` thành `* Không chọn nếu live 24/7`, thêm note đỏ `* Để trống nếu không Backup` cho `BOT backup`, rút mô tả dưới `Live Config`, và thay empty-state bảng live về bản ngắn gọn cùng nhịp với `Điều phối render`.
- 2026-04-10: Updated the user live workspace header and top KPI. `backend/app/store.py` now labels the first live summary tile as `BOT được cấp` and counts the user’s assigned live-stream BOT capacity directly from the BOT assignments used for both primary and backup selection. `backend/app/templates/user_live_dashboard.html` now adds a compact icon badge to the `Điều phối live` page header so the top bar feels less empty while staying inside the existing visual language. Verified with `python -m compileall backend/app`, a local `uvicorn` restart, and browser audit screenshot `output/user-live-header-kpi.png`.
- 2026-04-10: Refined the same `/app/live` screen again after visual feedback. The top header was reverted from the pill badge back to the original simple icon-plus-title treatment, while the two inner configuration section headers now carry light Lucide icon chips for `Nguồn phát` and `BOT và lên lịch`. Verified with `python -m compileall backend/app` and browser audit screenshot `output/user-live-header-revert-section-icons.png`.
- 2026-04-10: Reverted the temporary section-header icon chips inside `backend/app/templates/user_live_dashboard.html`, restoring `Nguồn phát` and `BOT và lên lịch` to their previous plain-title layout while keeping the already-restored top header and the `BOT được cấp` KPI unchanged. Verified with `python -m compileall backend/app`.
- 2026-04-10: Reworked the user `/app/live` table schema to follow the legacy live-stream information model while keeping the `Điều phối render` visual language. `backend/app/templates/user_live_dashboard.html` now prepares the live list with columns for action, BOT, group, stream title, stream key, backup BOT, live type, start/end time, duration, log, and status, and removes the temporary thumbnail block from the stream-title column per latest feedback. Verified with `python -m compileall backend/app`.
- 2026-04-10: Fixed worker bootstrap regression by aligning the browser bootstrap env with `google-chrome-stable`, forcing noninteractive apt behavior during bootstrap, and preserving failed install tasks in control-plane state instead of letting failed BOT installs disappear from the admin BOT list.
- 2026-04-10: Refined BOT bootstrap failure handling again: failed install/decommission tasks no longer remain as visible placeholder rows in `Danh sách BOT`; instead `backend/app/store.py` now drops them from `worker_operation_tasks`, keeps the admin notification, logs structured failure context, and skips restoring old `failed/completed` tasks after restart. `backend/app/worker_bootstrap.py` now logs full stack traces for install/decommission thread failures, while the earlier bootstrap fixes for `google-chrome-stable` and `DEBIAN_FRONTEND=noninteractive` remain in place.
- 2026-04-10: Repaired Vietnamese mojibake in the admin shell and BOT workspace by normalizing the affected UI/data strings in `backend/app/store.py` (menu labels, KPI titles, browser-session errors, live workspace copy, manager/BOT validation text, and related page titles). Also rechecked production install logs for `167.86.87.213` and confirmed the current add-BOT failure is still an SSH timeout from the app host, not a hidden bootstrap crash.
- 2026-04-10: Removed `Wants=network-online.target` from `infra/systemd/youtube-upload-worker.service` and downgraded the dependency to `After=network.target`. This avoids the recurring ~120s `systemd-networkd-wait-online` timeout seen on some Contabo VPS nodes during BOT bootstrap, while the worker keeps its own HTTP retry/re-register resilience after start.
- 2026-04-10: Hardened BOT add/remove lifecycle further without changing the UI contract: persisted install/decommission tasks can now be re-dispatched after an app restart, `awaiting_registration` and stale worker operations now fail on explicit timeouts instead of hanging indefinitely, and SSH bootstrap/decommission commands now have bounded execution time plus stderr/stdout excerpts for faster production debugging.
- 2026-04-10: Limited BOT installation concurrency on the shared app host. `backend/app/worker_bootstrap.py` now treats `WORKER_INSTALL_MAX_CONCURRENCY=2` as the default guardrail and only launches two SSH bootstrap threads at a time; additional install tasks remain `queued` until a running install finishes.
- 2026-04-10: Prevented long BOT names from breaking the admin BOT list layout. `backend/app/templates/admin/worker_index.html` now constrains the `BOT` column width and applies `overflow-wrap:anywhere` / `word-break:break-word` to worker names so long identifiers wrap inside the cell instead of forcing horizontal scroll.
- 2026-04-10: Tightened the same BOT list layout fix after live verification. The upload workspace variant of `backend/app/templates/admin/worker_index.html` now uses a dedicated fixed-layout table profile (`admin-table-upload-workers`), removes inherited `white-space: nowrap` at the row/cell level for that table, narrows the `BOT` and `Tác vụ` columns, and mirrors the wrap classes in JS-generated live-poll rows so long BOT names no longer re-expand the table after refresh.
- 2026-04-10: Split unfinished live-stream admin work out of the current `main` workspace by creating a dedicated `D:\Youtube_BOT_UPLOAD_live-youtube` worktree on branch `live-youtube`, copying the in-progress admin/live files there, and restoring `main` back to upload-only scope before redeploying.
- 2026-04-10: Fixed the stray mojibake separator in the admin `Cập nhật` columns by sanitizing `updated_meta` at render time in `backend/app/templates/admin/user_index.html` and `backend/app/templates/admin/user_role_list.html`, and kept the BOT-name wrapping fix isolated to upload workspace only in `backend/app/templates/admin/worker_index.html`.
- 2026-04-10: Removed the last live-workspace navigation leaks from the `main` upload workspace by dropping the admin sidebar item `Điều phối live stream` and the user nav item `Điều phối Live` in `backend/app/store.py`, and normalized both `updated_meta` sources there to the ASCII separator `|` so the admin timestamp column no longer renders mojibake glyphs before the date.
- 2026-04-10: Locked the post-login home for `admin` and `manager` to `/admin/user/index` by centralizing `ADMIN_DASHBOARD_HOME` inside `backend/app/routers/web.py` and routing `/`, `/login`, `/admin`, and `/admin/login` through that single landing path.
- 2026-04-10: Fixed a remaining admin/manager session regression where revisiting `/` could still fall into `/app` if an old `app_session` cookie was present. `backend/app/routers/web.py` now prioritizes `admin_session` over `app_session` in `/`, `/login`, and `/admin`, stops mirroring admin users into `/app`, and clears any stale matching `app_session` before redirecting back to `/admin/user/index`.
- 2026-04-10: Restored `/app` as a valid render-workspace entry point for `admin` and `manager` without reintroducing the old redirect bug. `backend/app/routers/web.py` now derives a temporary `AppSessionUser` from the existing `admin_session` only for the current `/app` request, clears any stale persisted `app_session`, and no longer writes admin/manager identities back into the app-session cookie.
- 2026-04-12: Fixed the extra top gap on admin `Danh sach BOT`. `backend/app/templates/admin/worker_index.html` now uses a local `worker-page-shell` rule to force the page section margin-top to `0`, removing the stray 24px offset introduced by the inline style block placed before the page `<section>`. Verified in the local browser after restarting `uvicorn`: `Nguoi dung`, `Danh sach Kenh`, and `Danh sach BOT` now all render with a `24px` gap from the top bar to the first panel, and the live BOT polling JS was rechecked with no code change needed because it only refreshes table rows.
- 2026-04-12: Added explicit width classes to every header cell in the admin `Danh sach BOT` table at `backend/app/templates/admin/worker_index.html`, so each column can now be tuned directly from the template without guessing inherited widths. The previously missing width classes were added for `Group`, `Ngay tao`, `So kenh`, `So user`, `Trang thai`, `RAM`, and `Luong`, while the existing BOT/Disk/Bandwith widths were normalized into the same per-column pattern. Verified with `python -m compileall backend/app`.
- 2026-04-12: Hard-locked the admin `Danh sach BOT` table widths to the user-approved pixel values by moving all 12 column widths into a `colgroup` inside `backend/app/templates/admin/worker_index.html`. The BOT and Group cells now wrap long text in both server-rendered rows and the live-poll JS row builder, so long names no longer push the table wider or create horizontal scrolling. Verified after a live poll cycle and with simulated very long BOT/Group names: `table-shell` stayed at `scrollWidth == clientWidth`.
- 2026-04-12: Simplified the admin `Danh sach Kenh` and `Danh sach Render` tables for the updated local UI pass. `backend/app/templates/admin/channel_index.html` now renders `User duoc gan` as plain wrapped text instead of pill badges, while `backend/app/templates/admin/render_index.html` removes the `Link nguon` column, drops the duplicate `Tao luc` line from `Thong tin job`, and makes the small `channel_id` line under the channel name clickable. `backend/app/store.py` now supplies `channel_link` for render rows so the render table can reuse the same public-channel linking pattern as the channel list.
- 2026-04-12: Hardened the admin `Danh sach Kenh` table against long text overflow. `backend/app/templates/admin/channel_index.html` now uses a dedicated fixed-layout table profile with a `colgroup`, removes truncation from the `Kenh` cell, and wraps long values in `Kenh`, `Manager`, `User duoc gan`, `BOT`, and `Nhom` with `overflow-wrap:anywhere` / `word-break:break-word`. The row action cluster can also wrap onto multiple lines when space is tight so the table no longer needs horizontal scrolling just because a few values are too long.
- 2026-04-12: Added named width classes to both admin tables so column sizing can be tuned in one place. `backend/app/templates/admin/channel_index.html` now exposes `channel-col-*` classes for every `Danh sach Kenh` column and uses them in both the `colgroup` and header cells. `backend/app/templates/admin/render_index.html` now exposes `render-col-*` classes for every `Danh sach job render` column, switches the table to a fixed-layout profile with text wrapping for long content, and renders a local-only demo row whenever the render list is empty on `127.0.0.1` / `localhost` / `::1`.
- 2026-04-12: Added one more hardening layer to prevent horizontal overflow in the admin `Danh sach Kenh` and `Danh sach job render` tables. Their dedicated table profiles now force `white-space: normal` on both header and body cells, so the current pixel widths stay authoritative and long content must wrap instead of expanding the table. A route/store audit also confirmed that deleting a render job from the user side removes it from the shared global `self.jobs` collection, so the same job disappears from the admin list as well.
- 2026-04-12: Refined the admin `DS User` screen for a channel so it matches the current admin visual language and uses the shared searchable select pattern. `backend/app/templates/admin/channel_users.html` now renders a richer channel header, clearer scope copy, and a searchable user picker powered by the existing `toolbar-select` enhancement. `backend/app/store.py` and `backend/app/routers/web.py` now separate admin vs manager assignment scope: admin can add any eligible user already assigned to the same VPS as the channel, while manager remains limited to users they manage.
- 2026-04-12: Rolled the admin `DS User` screen for a channel back to its simpler legacy layout after visual review, but kept the new role-aware assignment policy and only retained the searchable dropdown enhancement. `backend/app/templates/admin/channel_users.html` is back to the original panel structure with a search-enabled `toolbar-select`, while `backend/app/templates/admin/channel_index.html` and `backend/app/templates/admin/render_index.html` now force their page sections back to `margin-top: 0` so the visible gap from the top bar returns to the expected 24px.
- 2026-04-12: Tightened the add-user toolbar alignment on the admin channel-user screen so the searchable select and `Thêm user` button hug the right edge like the other admin filter/action toolbars. `backend/app/templates/admin/channel_users.html` now applies `sm:ml-auto sm:justify-end` to the add-user form wrapper without changing the restored legacy layout.
- 2026-04-12: Widened the searchable user select on the admin channel-user screen so the control matches the visual weight of the other admin filter toolbars. `backend/app/templates/admin/channel_users.html` now gives the add-user form a larger max width and enforces a `320px` minimum width for the select on small-and-up breakpoints while keeping the control group right-aligned.
- 2026-04-12: Restored the visual separator in the admin `Cập nhật` columns from the temporary ASCII pipe back to the intended bullet. `backend/app/store.py` now emits `updated_meta` with `•`, and both `backend/app/templates/admin/user_index.html` and `backend/app/templates/admin/user_role_list.html` normalize any mojibake bullet variants back to `•` instead of downgrading them to `|`.
- 2026-04-12: Hardened host deploy rollout for Python/runtime changes. `scripts/bootstrap_host.sh` no longer just copies the default web unit and calls `enable --now`; it now renders the target systemd unit from `APP_DIR`, `SERVICE_PORT`, and `SYSTEMD_SERVICE_NAME`, then explicitly `restart`s that service so code synced to disk is always picked up by a fresh Uvicorn process. Deployment docs and infra memory were updated with a live-preview example for `/opt/youtube-upload-lush-live` on port `8010`, and `.gitattributes` now pins `*.sh` / `infra/systemd/*.service` to LF so Linux deploy artifacts do not regress on Windows checkouts.
