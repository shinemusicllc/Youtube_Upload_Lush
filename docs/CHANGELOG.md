...
- 2026-04-13: Tightened live-worker storage hygiene after disk audits on the Contabo live VPS. `workers/agent/cleanup.py` now uses a shorter retention window for stale `worker-data/live-streams/*` directories, `workers/agent/main.py` lets janitor run more frequently on live workers, and live bootstrap defaults now persist those janitor settings while also stopping/disabling `xrdp` for `WORKER_RUNTIME_MODE=live` in `scripts/bootstrap_worker.sh` / `backend/app/worker_bootstrap.py`. This keeps orphaned `rendered.flv` artifacts and XRDP log growth from sitting on disk for hours after failed tests or abrupt worker restarts.
