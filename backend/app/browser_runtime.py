...
class BrowserRuntimeManager:
    ...
    def load_config(self) -> BrowserRuntimeConfig:
        profile_root = Path(
            os.getenv("BROWSER_SESSION_PROFILE_ROOT", str(self.data_dir / "browser-profiles"))
        )
        session_root = Path(
            os.getenv("BROWSER_SESSION_STATE_ROOT", str(self.data_dir / "browser-sessions"))
        )
        ...
        return BrowserRuntimeConfig(
            ...
            lifetime_minutes=max(5, int(os.getenv("BROWSER_SESSION_LIFETIME_MINUTES", "20"))),
            public_base_url=public_base_url,
            start_url=str(os.getenv("BROWSER_SESSION_START_URL", "https://studio.youtube.com")).strip()
            or "https://studio.youtube.com",
            display_base=max(30, int(os.getenv("BROWSER_SESSION_DISPLAY_BASE", "90"))),
            vnc_port_base=max(5900, int(os.getenv("BROWSER_SESSION_VNC_PORT_BASE", "15900"))),
            web_port_base=max(6080, int(os.getenv("BROWSER_SESSION_WEB_PORT_BASE", "16080"))),
            debug_port_base=max(9222, int(os.getenv("BROWSER_SESSION_DEBUG_PORT_BASE", "19220"))),
            bind_host=str(os.getenv("BROWSER_SESSION_BIND_HOST", "0.0.0.0")).strip() or "0.0.0.0",
            novnc_web_dir=Path(
                os.getenv("BROWSER_SESSION_NOVNC_WEB_DIR", "/usr/share/novnc")
            ),
    ...
    def ensure_available(self, config: BrowserRuntimeConfig) -> None:
        if not config.enabled:
            raise ValueError("Browser session chua duoc bat. Hay dat BROWSER_SESSION_ENABLED=1.")
        if sys.platform != "linux":
            raise ValueError("Browser session chi ho tro host Linux/Ubuntu.")
        if not config.public_base_url:
            raise ValueError("Thieu BROWSER_SESSION_PUBLIC_BASE_URL de mo noVNC cho nguoi dung.")
        if not config.novnc_web_dir.exists():
            raise ValueError(
                f"Khong tim thay noVNC web dir tai {config.novnc_web_dir}. Cai goi novnc tren host truoc."
            )
        if not shutil.which(config.chromium_bin):
            raise ValueError(
                f"Khong tim thay Chromium binary '{config.chromium_bin}'. Cap nhat BROWSER_SESSION_CHROMIUM_BIN."
            )
        for binary in ("Xvfb", "openbox", "x11vnc", "websockify"):
            if not shutil.which(binary):
                raise ValueError(f"Khong tim thay binary '{binary}'. Cai package browser runtime tren host.")
    ...
    def launch(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        if chromium.poll() is not None:
            raise ValueError("Chromium da thoat ngay sau khi khoi dong. Kiem tra chromium.log tren host.")
    ...
    def inspect(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        current = next(
            (
                ...
                if "studio.youtube.com" in str(item.get("url") or "")
            ),
