  ...
  const initChannelConnectButton = () => {
    ...
    const setLaunchState = (mode, session) => {
      ...
      if (mode === "ready") {
        ...
        launchDescription.innerHTML = "Đăng nhập Google rồi vào đúng trang <strong>YouTube Studio</strong> của kênh cần thêm.";
        return;
      }
      ...
      launchTitle.textContent = "Đang khởi tạo phiên đăng nhập";
      launchDescription.innerHTML = "Đang chuẩn bị phiên đăng nhập. Khi sẵn sàng, nút <strong>Đăng nhập Google</strong> sẽ sáng lên.";
    };
    ...
    closeSessionButton.addEventListener("click", async () => {
      ...
      const confirmed = window.confirm("Đóng phiên Chromium/noVNC này?");
      if (!confirmed) return;
